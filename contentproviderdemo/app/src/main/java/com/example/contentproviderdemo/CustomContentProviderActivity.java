...
public class CustomContentProviderActivity extends AppCompatActivity implements View.OnClickListener {
    ...
    private void addStudent(String studentName, String city) {
        if (isNotEmpty(studentName, city)) {
            ...
            contentValues.put(StudentDatabaseHelper.STUDENT_NAME_COLUMN_NAME, studentName);
            contentValues.put(StudentDatabaseHelper.STUDENT_CITY_COLUMN_NAME, city);

            Uri uri = contentResolver.insert(StudentProvider.CONTENT_URI_ALL_STUDENTS, contentValues);
            if (uri != null) {
        ...
        if (isNotEmpty(id)) {
            ...
            if (cursor != null && cursor.getCount() > 0) {
                ...
                while (cursor.moveToNext()) {
                    String studentId = cursor.getString(cursor.getColumnIndex(StudentDatabaseHelper.STUDENT_ID_COLUMN_NAME));
                    String studentName = cursor.getString(cursor.getColumnIndex(StudentDatabaseHelper.STUDENT_NAME_COLUMN_NAME));
                    String studentCity = cursor.getString(cursor.getColumnIndex(StudentDatabaseHelper.STUDENT_CITY_COLUMN_NAME));
    ...
    private void fetchAllStudents() {
        ContentResolver contentResolver = getContentResolver();
        Cursor cursor = contentResolver.query(StudentProvider.CONTENT_URI_ALL_STUDENTS, null, null, null, null);
